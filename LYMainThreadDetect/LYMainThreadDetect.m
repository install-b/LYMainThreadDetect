...
#pragma mark - create source timer
...
static void ping_main_thread(uint64_t interval)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
         // 超时之前 响应 取消
        if (pongTimer) {
            dispatch_source_cancel(pongTimer);
        }
    });
}
#pragma mark - close/open/init detector
